...
axios.interceptors.response.use(
  ...
  err => {
    if(err.response === undefined) {
      return {code: 500, msg: err.response.data.error_description || '未收到服务器响应', status: 504};
    }else if (err.response.status === 404) {
      return {code: 404, msg: "请求接口未找到", status: err.response.status};
    } else if (err.response.status === 401) {
      localStorage.clear()
      sessionStorage.clear()
      history.push('/login?status=401')
      ...
      return {code: 500, msg: err.response.data.error_description || "服务器异常", status: err.response.status};
    }else {
      return Promise.reject({code: err.response.status, msg: err.response.data.error_description || err.response.statusText, status: err.response.status});
    }
  }
);
